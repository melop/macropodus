...
# 假设 list 是包含文件名的向量
species_list <- c("MHKhn", "MHKjx", "MHKmlh_qns", "MHKmls_pop1", "MHKmls_pop2", "MOPdsy", "MOPdxs_sg", "MOPfq_pt")
...
# 定义筛选和统计区间数量的函数
count_intervals <- function(file_path, marker_file_path, sp1, sp2) {
  tryCatch({
    # 读取 marker 文件
    marker_data <- read.table(marker_file_path, header = TRUE, sep = "\t")
    ...
    if (nrow(invalid_markers) == 0) {
      cat("invalid_markers 为空，不进行后续窗口格式转换操作。\n")
      file_data <- read.table(file_path, header = FALSE, sep = "\t")
      return(nrow(file_data))
    }
    
    # 检查 invalid_markers$win 列是否存在 NA 值
    if (any(is.na(invalid_markers$win))) {
      cat("invalid_markers$win 列存在 NA 值，去除 NA 值。\n")
      invalid_markers <- invalid_markers[!is.na(invalid_markers$win), ]
    }
    
    # 进行窗口格式转换
    invalid_windows <- do.call(rbind, strsplit(as.character(invalid_markers$win), split = "\\."))
    ...
    if (!is.null(dim(invalid_windows)) && nrow(invalid_windows) > 0 && ncol(invalid_windows) >= 3) {
      ...
      for (col in 2:3) {
        invalid_windows[, col] <- as.integer(invalid_windows[, col])
      }
      colnames(invalid_windows) <- c("chr", "start", "end")
    } else {
      cat("invalid_windows 行数或列数不足，不进行后续类型转换操作。\n")
    }
    
    # 读取要统计的文件
    file_data <- read.table(file_path, header = FALSE, sep = "\t")
    
    if (exists("invalid_windows") && nrow(invalid_windows) > 0) {
      # 删除文件中与无效窗口相同的行
      valid_rows <- !apply(file_data[, 1:3], 1, function(x) any(apply(invalid_windows, 1, function(y) all(x == y))))
      valid_data <- file_data[valid_rows, ]
      return(nrow(valid_data))
    } else {
      return(nrow(file_data))
    }
  }, error = function(e) {
    cat(paste0("在处理组合 ", sp1, " 和 ", sp2, " 时出现错误：", conditionMessage(e), "\n"))
    return(NA)
  })
}
...
for (i in 1:(N - 1)) {
  for (j in (i + 1):N) {
    ...
    # 统计区间数量
    k <- count_intervals(file_both_balanced, marker_file, sp1, sp2)
    m <- count_intervals(file_both_balanced, marker_file, sp1, sp2) + count_intervals(file_only_sp1_balanced, marker_file, sp1, sp2)
    N_total <- count_intervals(file_both_balanced, marker_file, sp1, sp2) + count_intervals(file_only_sp1_balanced, marker_file, sp1, sp2) + 
      count_intervals(file_only_sp2_balanced, marker_file, sp1, sp2) + count_intervals(file_both_no_balanced, marker_file, sp1, sp2)
    n <- count_intervals(file_both_balanced, marker_file, sp1, sp2) + count_intervals(file_only_sp2_balanced, marker_file, sp1, sp2)
