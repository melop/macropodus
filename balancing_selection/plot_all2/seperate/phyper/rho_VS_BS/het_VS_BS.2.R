...
# 读取 10kb 窗口重组率文件
genome_10kb <- read.table("/data2/projects/zwang/m.hk/ROH/BetaScan/rho_VS_het/het/average_heterozygosity.txt", header = T)
...
check_BS_recombination <- function(BS_data, genome_data) {
  ...
  BS_average_heterozygositys <- numeric(nrow(BS_data))
  ...
  for (i in seq_len(nrow(BS_data))) {
    ...
    if (nrow(included_windows) > 0) {
      BS_average_heterozygositys[i] <- mean(included_windows$average_heterozygosity)
    } else {
      # 如果没有包含的 10kb 窗口，将重组率设为 NA
      BS_average_heterozygositys[i] <- NA
    }
  }
  
  # 将重组率添加到 50kb 倍数窗口数据中
  BS_data$average_heterozygosity <- BS_average_heterozygositys
  
  # 去除没有包含 10kb 窗口的行
  BS_data <- BS_data[!is.na(BS_data$average_heterozygosity), ]
...
# 进行显著性比较
test_result <- wilcox.test(average_heterozygosity ~ source, data = final_data)
cat("BS 与 BS + 非BS 窗口杂合度的 Wilcoxon 秩和检验结果:\n")
print(test_result)
...
# 绘制箱线图
p <- ggplot(final_data, aes(x = source, y = average_heterozygosity)) +
  geom_boxplot() +
  labs(y = "Average Heterozygosity", title = "Comparison of Average Heterozygosity between BS and BS + Non-BS Regions") +
  theme_minimal() +
  theme(
...
# 获取 y 轴的最大值
y_max <- max(final_data$average_heterozygosity, na.rm = TRUE)
