...
p <- ggplot(final_data, aes(x = source, y = average_heterozygosity)) +
  geom_boxplot() +
  labs(y = "Average Heterozygosity", title = "Comparison of Average Heterozygosity between BS and Non-BS Regions") +
  theme_minimal() +
  theme(
...
print(p)
# 保存为 PDF 文件
ggsave("average_heterozygosity.BS_VS_NonBS.pdf", plot = p, width = 10, height = 8)
