...
pdf(file = "Balance_map_Het.phyper_test.pdf", width = 10, height = 8)
...
Heatmap(mat_transformed,
        ...
        row_names_gp = gpar(col = row_label_colors, fontsize = 20),  # 设置行标签的颜色和字体大小
        column_names_gp = gpar(col = col_label_colors, fontsize = 20),  # 设置列标签的颜色和字体大小
        heatmap_legend_param = list(title = "-log10(p)", title_gp = gpar(fontsize = 24), labels_gp = gpar(fontsize = 20)),  # 设置图例标题和标签的字体大小
        row_title = "Population",  # 设置纵轴标题
        row_title_gp = gpar(fontsize = 24, fontface = "bold"),  # 设置纵轴标题的字体大小和样式
        column_title = "Population",  # 设置横轴标题
        column_title_gp = gpar(fontsize = 24, fontface = "bold")  # 设置横轴标题的字体大小和样式
)
