...
# 开启 PDF 图形设备
pdf("2pops.shearedHET.distribution.pdf", width = 24, height = 6)
...
for (i in seq_along(file_list)) {
  for (j in i:length(file_list)) {
    ...
    if (file.exists(file_name)) {
      ...
      labs(title = paste("Histogram of", file_list[i], "-", file_list[j]), x = "Length", y = "Frequency") +
      scale_x_continuous(expand = c(0, 0), limits = c(0, max_len)) +
      theme_minimal() +
      theme(
        # 设置标题字体大小
