...
arrHomFiles <- Sys.glob("2pops.intersected.HET.balanced.bed");
...
for (sF in arrHomFiles) {
  ...
  datThis$Group <- gsub('.intersected.HET.balanced.bed','', sF, fixed = T);
...
sOut <- paste0("2.2pops.sharedHET.GO.tsv.txt");
datOut <- oEnrichRet@result[ oEnrichRet@result$pvalue <=0.05, ] #0.05
