...
write.table(all_results, "DP_map_Cutoff.1e-3.9.txt", sep = "\t", row.names = FALSE)


#-------------------------------------------------------------------------------------
all_results <- read.table("DP_map_Cutoff.1e-3.9.txt", sep = "\t", header = T)
...
# 获取Max_y列的范围（最小值和最大值）
# 直接指定Max_y的范围
max_y_range <- c(0.966, 0.9751)

# 指定保存的PDF文件名，可根据实际需求修改
pdf_file_name <- "DSY-1.Cutoff_map_accuracy_differentDP.1e-3.9.pdf"
