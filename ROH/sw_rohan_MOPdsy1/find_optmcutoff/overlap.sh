...
cat ./test.1e-3.9.tsv | awk '{if ($1!="chr") {print $1"\t"$3-1"\t"$4} }' > roh.bed
...
#	echo $sD
	...
	#bedtools intersect -a $sD.rohan.het.bed -b roh.bed -wa -f 0.99 > $sD.roh.wins.bed #previous
	bedtools intersect -a $sD.rohan.het.bed -b roh.bed -wo > $sD.roh.wins.bed
}&
done
wait
