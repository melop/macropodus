...
# 设置保存PDF文件的路径和文件名
pdf_file_path <- "/data2/projects/zwang/m.hk/ROH/simulation2/plot1/all4.pdf"
...
for (file_path in file_list) {
  ...
  density_plot_with_percentiles <- density_plot +
    geom_vline(xintercept = quantile(data, probs = c(0.05, 0.95)),
               color = "red",
               linetype = "dashed") +
    annotate("text",
             x = quantile(data, probs = c(0.05, 0.95)),
             y = 8e-07,
             label = c("5% percentile", "95% percentile"),
             color = "red",
             size = 10)
...
# 加载所需的包
library(ggplot2)
library(gridExtra)  # 用于将多个图组合在一页上

# 用于存储所有图形对象的列表
plot_list <- list()

pdf(pdf_file_path, width = 8.27, height = 11.69)
...
for (file_path in file_list) {
  ...
  density_plot <- ggplot(df, aes(x = value)) +
    ...
    theme_minimal() +
    theme(
      text = element_text(size = 4)  # 设置所有文字的大小
    )
  ...
  # 将当前的data2数据添加到汇总数据框中
  if (!exists("all_data2")) {
    all_data2 <- data2
  } else {
    all_data2 <- rbind(all_data2, data2)
  }
