...
library(GENESPACE)
runwd <- file.path("/data2/projects/zwang/macropodus_compare/synteny/genespace_RC/rundir")
#list.files(runwd, recursive = T, full.names = F)

gpar <- init_genespace(
  genomeIDs = c('MacropodusOpercularis', 'MacropodusHongkongensis', 'BettaSplendens',  'AnabasTestudineus',  'OreochromisNiloticus', 'GasterosteusAculeatus'),
  speciesIDs = c('Macropodus_opercularis', 'Macropodus_hongkongensis', 'Betta_splendens',  'Anabas_testudineus', 'Oreochromis_niloticus', 'Gasterosteus_aculeatus'),
  versionIDs = c("1.0", "1.0", "104", "104", "104", "104"),
  ploidy = rep(1,6),
  diamondMode = "default",
