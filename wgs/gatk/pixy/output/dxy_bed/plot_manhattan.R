setwd("/fast3/group_crf/home/g20wangzhx36/m.hk/Structure/Pixy/output/dxy_bed")
...
# 创建一个pdf文件用于保存所有的图
pdf("presentive_MHK_MOP.dxy.manhattan.pdf", width = 18, height = 6)
...
for (file_base_name in file_list) {
  ...
  data <- read.table(file_name, header = FALSE, sep = "\t", 
                     col.names = c("chromosome", "start", "end", "dxy"))
  ...
  # 进一步筛选，去除dxy为NA的行
  filtered_data <- filtered_data[!is.na(filtered_data$dxy), ]
  ...
  # 找到当前文件dxy的top1%的值
  top_1_percent_value <- quantile(filtered_data$dxy, 0.99, na.rm = TRUE)
  
  # 根据dxy与top1%值的比较，设置颜色列
  filtered_data <- filtered_data %>%
    mutate(point_color = ifelse(dxy >= top_1_percent_value, as.character(chromosome), "gray"))
  # 将小于阈值的点颜色设置为灰色
  ...
  # 绘制曼哈顿图
  p <- ggplot(filtered_data, aes(x = adjusted_midpoint, y = dxy, color = point_color)) +
    geom_point(size = 1, alpha = 0.8) +
    labs(x = "Chromosome", y = "dxy", title = file_base_name) +
    theme_bw() +
