...
dat <- read.csv("MOP.kinship.tsv", header = T, sep = "\t")

pops <- read.csv("MOP.sample_names.csv", header = F, sep = ",")
...
write.table(output_df, "MOP.average_kinship.txt", sep = "\t", na = "NA", quote = FALSE, row.names = FALSE)




#-----------------------------------------------------------------------之前用的/data2/projects/dyao/macropodus/MOP/kinship/MOP/kinship.tsv
