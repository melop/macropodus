...
fitMKmodel <- function(alpha_trimmed, f_trimmed, res)
{
  ...
  mod <- tryCatch({
    st <- expand.grid(const_a=seq(-30,30,length.out=res + 1), const_b=seq(-30,30,length.out=res), const_c=seq(1,10,length.out=res + 1))
...
asymptoticMK <- function(d0, d, xlow, xhigh, df, true_alpha=NA, output="table", arrYLims=NULL,  ...)
{
  ...
  else
  {
    ...
    points(x=f, y=alpha, col=ifelse(trim, fit_color, "#999999"), pch=19, cex=0.7)
...
asymptoticMKOverlay <- function(lsListArgs )
{
  ...
  for(sPop in names(lsListArgs)) {
    ...
    bPlotCI <- lsListArgs[[sPop]]$plotCI;
    
    message(paste0("\nPlotting ", sPop, "d0=",d0, "d=", d, "xlow=",xlow, "xhigh=", xhigh, "true_alpha=", true_alpha,"\n" ));
    ...
    result_df <- data.frame(model=(if ((length(mod1) == 0) || linear_better) "linear" else "exponential"), a=const_a, b=const_b, c=const_c, alpha_asymptotic=alpha_1_est, CI_low=alpha_1_low, CI_high=alpha_1_high, alpha_original=alpha_nonasymp, row.names=NULL)
      ...
      if (bPlotCI ) {
        polygon(x=c(-1, -1, 2.0, 2.0), y=c(alpha_1_low, alpha_1_high, alpha_1_high, alpha_1_low), col=rgb(col2rgb(fit_color)[1]/255, col2rgb(fit_color)[2]/255, col2rgb(fit_color)[3]/255, 0.1), border=NA)
      }
      #if (!is.na(alpha_nonasymp))
      #  abline(h=alpha_nonasymp, lty=3, col="#777777")
