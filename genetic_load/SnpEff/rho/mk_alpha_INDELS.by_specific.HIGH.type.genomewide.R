...
arrImpact <- c( "frameshift_variant", "stop_gained", "stop_lost", "start_lost", "splice_donor_variant", "splice_acceptor_variant"); #"3_prime_UTR_variant", "5_prime_UTR_variant",
lsPlotData <- list();

sOut <- "asymtotoic_mk_indels.byspecifictypes.HIGH.paired.txt"
...
for(sImpact in arrImpact) {
  ...
  lsPlotData[[sImpact]]$arrYLims <- c(-40, 1);
...
# lsPlotData[[1]]$col <- 'red'; #frameshift_variant
# lsPlotData[[2]]$col <- 'blue'; #stop_gained
# lsPlotData[[3]]$col <- 'green1'; #stop_lost
# lsPlotData[[4]]$col <- 'yellow1';#start_lost
# lsPlotData[[5]]$col <- 'violet'; #splice_donor_variant
# lsPlotData[[6]]$col <- 'cyan';#splice_acceptor_variant
...
legend("bottomright", inset=.05, title="High Impact Type", arrImpact,
