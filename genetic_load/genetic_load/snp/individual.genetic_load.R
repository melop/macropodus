setwd("/data2/projects/zwang/m.hk/Snpeff/allpops/rho_V2.0/filtered_SOL_3.5/Individual/snp")
...
dat <- read.table("allpops.refMHK.NONSYN.polarized.out.filtered.withconsurf.bed", header=F, sep="\t")
dat <- dat[ , c(1:3, 6:ncol(dat) ) ]
p_NA <- colSums(is.na(dat[, c(10:(ncol(dat)-4))]))/nrow(dat)
print(p_NA)
#dat <- dat[complete.cases(dat), ]
dat_HIGH <- dat[dat$V76==9,] #consurf scores == 9
dat_HIGH <- dat_HIGH[,c(1:(ncol(dat_HIGH)-4))]

length_liftover <- 30956424 # liftover上的去除掉SV文件非双多态性的经过DP筛选的cds位点数 /data2/projects/zwang/test/DeepVatiant/DPcutoff_for_all/liftover_for_individual/snp_forGATK/filter_DP.R
