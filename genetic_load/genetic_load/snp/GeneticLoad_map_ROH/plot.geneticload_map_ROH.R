...
print(p)
# 保存为 PDF
ggsave("Linear_regression_analysis_between_geneticload_ROHlength.HIGH.pdf", plot = p, width = 12, height = 8)
#-------------------------------------------------------------------------------------混合线型模型。summary(model),将t value转换为斜率p value, p = 2*(1-pnorm(abs(t)))。0.3370552
# 计算个体的 ROH 长度总和函数
calculate_roh_length <- function(pop, id) {
  roh_dir <- "/fast3/group_crf/home/g20wangzhx36/m.hk/ROH/ROHan/runs_of_hom/"
  roh_file <- file.path(roh_dir, pop, paste(id, ".ROH.bed", sep = ""))
