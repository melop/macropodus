...
for (comb in pop_combinations) {
  ...
  if ((startsWith(i, "MHK") && startsWith(j, "MHK")) || (startsWith(i, "MOP") && startsWith(j, "MOP"))) {
    ...
    test_result <- wilcox.test(data_i, data_j, exact = FALSE)
...
print(p)
# 保存图形为 PDF
ggsave("genetic_load_boxplot.snp.with_signinf.pdf", plot = p, width = 12, height = 8, units = "in")
...
# 进行 Wilcoxon 秩和检验
test_result <- wilcox.test(data_pop1, data_pop2)
p_value <- test_result$p.value
