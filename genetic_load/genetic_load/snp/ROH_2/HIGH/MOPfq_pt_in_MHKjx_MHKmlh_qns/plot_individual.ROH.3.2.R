#------------------------------------------------------------------------------------
# 设置工作目录
setwd("/data2/projects/zwang/m.hk/Snpeff/all_MHK_MOP/rho_V2.0/filtered_SOL_3.5/Individual/snp/ROH_2/HIGH/MOPfq_pt_in_MHKjx_MHKmlh_qns")
...
#  print(individual2)
  ...
  reference_population <- dat$population[dat$V1 == individual2]
...
if (nrow(result_df) == 0) {
  ...
  p <- ggplot(result_df, aes(x = reference_population, y = normalized_ratio, fill = reference_population)) +
    ...
    labs(x = "reference_population", y = "GLoF/GLoR", fill = "") +
  ...
  ggsave("allpops.refMHK.NONSYN.Individuals_Genetic_Load_Ratio_of_ROH-HET.MOPfqpt_on_MHKjxMHKmlhqns.3.pdf", plot = p, width = 8, height = 6)
  print(p)
}
