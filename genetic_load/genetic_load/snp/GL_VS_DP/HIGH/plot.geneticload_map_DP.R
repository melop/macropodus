#------------------------------------------------------------------
# 设置工作目录
setwd("/data2/projects/zwang/m.hk/Snpeff/all_MHK_MOP/rho_V2.0/filtered_SOL_3.5/Individual/snp/GL_VS_DP/HIGH")
...
# 读取表格数据
table1 <- read.table("genetic_load_of_individual.HIGH.allpops.refMHK.txt", header = T)
...
for (i in 1:nrow(data_table)) {
  ...
  all_individuals_data <- rbind(all_individuals_data, data.frame(genetic_load = genetic_load, id = id, population = pop, DP = DP))
}
...
# 进行线性回归并绘图
p <- ggplot(all_individuals_data, aes(x = DP, y = genetic_load)) +
  geom_point(aes(color = population)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "DP", y = "Genetic Load") +
  ggtitle("Linear regression analysis between genetic load and DP") +
  theme_minimal() +
  ...
  annotate("text", x = mean(range(all_individuals_data$DP)), y = max(all_individuals_data$genetic_load) * 0.8, label = "p=0.5626", size = 8) +
  scale_color_manual(values = color_mapping)


print(p)
# 保存为 PDF
ggsave("Linear_regression_analysis_between_geneticload_DP.group_color.snp.HIGH.pdf", plot = p, width = 12, height = 8)

#-------------------------------------------------------------------------------------混合线型模型。summary(model),将t value转换为斜率p value, p = 2*(1-pnorm(abs(t)))。0.5626
...
# 构建线性混合模型
model <- lmer(genetic_load ~ DP_std + (1 | population), data = all_individuals_data)
...
# 进行绘图，设置字体大小
ggplot(all_individuals_data, aes(x = DP_std, y = genetic_load)) +
  geom_point(aes(color = population)) +
  geom_line(data = predicted_data, aes(x = DP_std, y = genetic_load, color = population), size = 1) +
  labs(x = "Standardized DP", y = "Genetic Load") +
  ggtitle("Linear mixed model analysis between genetic load and standardized DP") +
  theme_minimal() +
