# 设置工作目录
setwd("/data2/projects/zwang/m.hk/Snpeff/all_MHK_MOP/rho_V2.0/filtered_SOL_3.5/Individual/snp/Ration_of_HomHet/Consurf9")
...
dat <- read.table("genotype_proportions.Consurf9.txt", header=TRUE) 
# 添加种群信息
...
# 绘制箱线图
p <- ggplot(dat , aes(x = population, y = Proportion, fill = population)) +
  geom_boxplot() +
  labs(title = "Proportion of Hom in Genetic Load",
       x = "Population",
       y = "Proportion of Hom") +
...
# 保存图形为PDF
pdf_file <- "allpops.refMHK.NONSYN.Consurf9.Individuals_Genetic_Load.ProportionHom.Boxplot.pdf"
