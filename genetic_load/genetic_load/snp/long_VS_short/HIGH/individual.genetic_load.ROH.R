...
file_list <- list.files(path = "/data2/projects/zwang/m.hk/Snpeff/all_MHK_MOP/rho_V2.0/filtered_SOL_3.5/Individual/snp/long_VS_short/HIGH/bed", pattern = "allpops\\.refMHK\\.NONSYN\\.filtered\\.(.*)\\.(shortROH|longROH)\\.bed$", full.names = TRUE)

# 提取个体名
individuals <- unique(gsub("allpops\\.refMHK\\.NONSYN\\.filtered\\.(.*?)\\.(shortROH|longROH)\\.bed$", "\\1", basename(file_list)))
...
for (individual in individuals) {
  # 构建ROH和HET文件名
  roh_file <- paste0("allpops.refMHK.NONSYN.filtered.", individual, ".shortROH.bed")
  het_file <- paste0("allpops.refMHK.NONSYN.filtered.", individual, ".longROH.bed")
  
  # 完整路径
  roh_path <- file.path("/data2/projects/zwang/m.hk/Snpeff/all_MHK_MOP/rho_V2.0/filtered_SOL_3.5/Individual/snp/long_VS_short/HIGH/bed", roh_file)
  het_path <- file.path("/data2/projects/zwang/m.hk/Snpeff/all_MHK_MOP/rho_V2.0/filtered_SOL_3.5/Individual/snp/long_VS_short/HIGH/bed", het_file)
  ...
  # 完整路径
  interval_roh_path <- file.path("/data2/projects/zwang/m.hk/Snpeff/all_MHK_MOP/rho_V2.0/filtered_SOL_3.5/Individual/snp/long_VS_short/HIGH/bed", interval_roh_file)
  interval_het_path <- file.path("/data2/projects/zwang/m.hk/Snpeff/all_MHK_MOP/rho_V2.0/filtered_SOL_3.5/Individual/snp/long_VS_short/HIGH/bed", interval_het_file)
