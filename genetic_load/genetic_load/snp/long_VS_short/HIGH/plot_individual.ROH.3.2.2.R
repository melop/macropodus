...
# 创建一个PDF文件来保存所有的图形
pdf("allpops.refMHK.NONSYN.HIGH.Individuals_Genetic_Load_Ratio_of_shortROH-longROH.2.pdf", width = 16, height = 10)
...
for (i in seq_along(results)) {
  ...
  for (target_pop in target_populations) {
    ...
    if (nrow(dat_temp) > 0) {
      target_mean_roh <- mean(dat_temp$normalized_genetic_load_shortROH, na.rm = TRUE)
      target_mean_het <- mean(dat_temp$normalized_genetic_load_longROH, na.rm = TRUE)
...
if (nrow(plot_data) == 0) {
  ...
  p <- ggplot(plot_data, aes(x = Population, y = Ratio, fill = Target_pop)) +
    ...
    labs(x = "Target Population", y = "RshortROH/RlongROH", fill = "Reference Population") +
