...
dat <- read.table("genetic_load_of_individual.HIGH.allpops.refMHK.txt", header=TRUE) 
# 添加种群信息
#dat$population <- rep(c("MHKhn", "MHKmlh_qns", "MHKmls_pop1", "MHKmls_pop2", "MOPdsy", "MOPdxs_sg", "MOPhn"), times = c(8, 7, 9, 13, 7, 10, 7))
...
# 过滤掉 MHKhk 种群
#dat_filtered <- subset(dat, population != "MHKhk")
#dat_filtered <- subset(dat_filtered, population != "MOPgl")
...
# 保存图形为PDF
pdf_file <- "allpops.refMHK.NONSYN.HIGH.Individuals_Genetic_Load_Boxplot.pdf"
ggsave(pdf_file, plot = p, width = 10, height = 6)
#-------------------------------------------------------------------------------------------
dat <- read.table("genetic_load_of_individual.Consurf9.allpops.refMHK.txt", header=TRUE) 
# 添加种群信息
#dat$population <- rep(c("MHKhn", "MHKmlh_qns", "MHKmls_pop1", "MHKmls_pop2", "MOPdsy", "MOPdxs_sg", "MOPhn"), times = c(8, 7, 9, 13, 7, 10, 7))
dat$population <- rep(c("MOPdsy", "MOPfq_pt", "MOPgl", "MHKhk", "MHKhn", "MHKjx", "MHKmls_pop1", "MHKmls_pop2", "MHKmls_pop1", "MHKmls_pop2", "MHKmls_pop1", "MHKmls_pop2", "MHKmlh_qns", "MOPdxs_sg", "MOPhn", "MOPld", "MOPdxs_sg", "MOPfq_pt", "MHKmlh_qns", "MOPyn"), 
                      times = c(7, 1, 4, 3, 8, 4, 4, 4, 3, 1, 2, 8, 3, 2, 7, 5, 8, 2, 4, 5))
...
# 保存图形为PDF
pdf_file <- "allpops.refMHK.NONSYN.Consurf9.Individuals_Genetic_Load_Boxplot.pdf"
ggsave(pdf_file, plot = p, width = 10, height = 6)
