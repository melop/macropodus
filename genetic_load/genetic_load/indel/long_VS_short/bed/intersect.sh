...
GL=./allpops.refMHK.SV.polarized.out.filtered.bed
for i in $sPath/*.ROH.bed; do
    ...
    # 判断 short 文件是否存在
    if [ -f "$short" ]; then
        bedtools intersect -a $GL -b $short > allpops.refMHK.SV.filtered.$sStem.shortROH.bed
    else
    ...
    # 判断 long 文件是否存在
    if [ -f "$long" ]; then
        bedtools intersect -a $GL -b $long > allpops.refMHK.SV.filtered.$sStem.longROH.bed
    else
