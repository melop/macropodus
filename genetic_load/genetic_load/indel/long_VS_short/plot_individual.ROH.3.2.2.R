...
# 创建一个PDF文件来保存所有的图形
pdf("allpops.refMHK.SV.Individuals_Genetic_Load_Ratio_of_shortROH-longROH.2.pdf", width = 16, height = 10)
...
# 合并目标种群的数据
# 目标种群列表
target_populations <- c("MOPhn", "MOPdsy", "MHKhn", "MHKhk")
...
for (i in seq_along(results)) {
  ...
  for (target_pop in target_populations) {
    ...
    if (nrow(dat_temp) > 0) {
      target_mean_roh <- mean(dat_temp$normalized_genetic_load_longROH, na.rm = TRUE)
      target_mean_het <- mean(dat_temp$normalized_genetic_load_shortROH, na.rm = TRUE)
  ...
  p <- ggplot(plot_data, aes(x = Population, y = Ratio, fill = Target_pop)) +
    ...
    geom_text(data = summary_stats,
              aes(label = paste("p =", sprintf("%.3f", p_value)),
                  x = Population, y = Q1-0.07,
                  group = interaction(Population, Target_pop)),
              position = position_dodge(width = 0.8),  # 根据箱线图的位置进行调整
              vjust = 0, size = 10, hjust = 0.5, angle = 0, inherit.aes = FALSE) +
    labs(x = "Focal Population", y = "RshortROH/RlongROH", fill = "Reference Population") +
    ...
    theme_minimal() +
    theme(
      axis.text.x = element_text(size = 28),  # x轴刻度标签的字体大小
      axis.text.y = element_text(size = 28),  # y轴刻度标签的字体大小
      axis.title.x = element_text(size = 28),  # x轴标题的字体大小
      axis.title.y = element_text(size = 28),  # y轴标题的字体大小
      legend.text = element_text(size = 28),  # 图例文本的字体大小
      legend.title = element_text(size = 28),  # 图例标题的字体大小
      plot.title = element_text(size = 28, face = "bold")  # 图形标题的字体大小和加粗样式
    ) +
