...
data_table <- data_table[!(data_table$pop %in% c("MHKhk", "MOPgl", "MHKhn")), ] #这两个种群没有ROH文件或者仅有一个个体存在ROH文件
...
# 定义分组
group1 <- c("MOPdsy", "MOPdxs_sg", "MOPhn", "MOPyn")
...
print(p)
# 保存为 PDF
ggsave("Linear_regression_analysis_between_geneticload_ROHlength.withoutMHKhn.group_color.pdf", plot = p, width = 12, height = 8)
