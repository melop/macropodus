...
data_table <- data_table[!(data_table$pop %in% c("MHKhk", "MOPgl", "MHKhn")), ] #这两个种群没有ROH文件或者仅有一个个体存在ROH文件， 去掉最高遗传载荷的MHKhn
...
p <- ggplot(all_individuals_data, aes(x = roh_length_Mb, y = genetic_load)) +
  ...
  annotate("text", x = mean(range(all_individuals_data$roh_length_Mb)), y = max(all_individuals_data$genetic_load) * 0.8, label = "p=0.0219", size = 8)
...
print(p)
# 保存为 PDF
ggsave("Linear_regression_analysis_between_geneticload_ROHlength.withoutMHKhn.pdf", plot = p, width = 12, height = 8)
#-------------------------------------------------------------------------------------混合线型模型。summary(model),将t value转换为斜率p value, p = 2*(1-pnorm(abs(t)))。0.02190564
# 计算个体的 ROH 长度总和函数
calculate_roh_length <- function(pop, id) {
  roh_dir <- "/fast3/group_crf/home/g20wangzhx36/m.hk/ROH/ROHan/runs_of_hom/"
  roh_file <- file.path(roh_dir, pop, paste(id, ".ROH.bed", sep = ""))
