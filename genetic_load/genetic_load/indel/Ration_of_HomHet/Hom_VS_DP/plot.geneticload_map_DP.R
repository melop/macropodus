#------------------------------------------------------------------
# 设置工作目录
setwd("/data2/projects/zwang/m.hk/Snpeff/all_MHK_MOP/rho_V2.0/filtered_SOL_3.5/Individual/indel/Ration_of_HomHet/Hom_VS_DP")
...
# 读取表格数据
table1 <- read.table("genotype_proportions.txt", header = T)
...
p <- ggplot(all_individuals_data, aes(x = DP, y = HomPP)) +
  ...
  annotate("text", x = mean(range(all_individuals_data$DP)), y = max(all_individuals_data$HomPP) * 0.8, label = "p=0.0082", size = 8) +
  scale_color_manual(values = color_mapping)


print(p)
# 保存为 PDF
ggsave("Linear_regression_analysis_between_HomPP_DP.group_color.indel.pdf", plot = p, width = 12, height = 8)
