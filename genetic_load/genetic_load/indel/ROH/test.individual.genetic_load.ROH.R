...
for (individual in individuals) {
  ...
  if (file.exists(roh_path) && file.exists(het_path)) {
    ...
    genetic_load_ratio <- normalized_genetic_load_roh / (normalized_genetic_load_het + 1e-10) # 加上1e-6以避免除以零
    # 保存结果
    output_file <- paste0("genetic_load_ratio_", individual, ".txt")
    write.table(genetic_load_ratio, file = output_file, col.names = FALSE, row.names = TRUE, quote = FALSE)
    
    # 打印结果
    print(paste("Processed:", individual))
    print(genetic_load_ratio)
  } else {
