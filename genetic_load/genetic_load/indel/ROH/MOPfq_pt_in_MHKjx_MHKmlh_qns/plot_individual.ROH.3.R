#------------------------------------------------------------------------------------
# 设置工作目录
setwd("/data2/projects/zwang/m.hk/Snpeff/all_MHK_MOP/rho_V2.0/filtered_SOL_3.5/Individual/indel/ROH/MOPfq_pt_in_MHKjx_MHKmlh_qns")
...
# 创建一个PDF文件来保存所有的图形
pdf("allpops.refMHK.SV.Individuals_Genetic_Load_Ratio_of_ROH-HET.MOPfqpt_on_MHKjxMHKmlhqns.pdf", width = 10, height = 6)
...
specified_indices <- c(8, 75, 76) # FJ PT SZ
# 存储参考个体比值的列表
...
# 合并目标种群的数据
# 目标种群列表
target_populations <- c("MHKjx", "MHKmlh_qns")
...
if (nrow(plot_data) == 0) {
  ...
  p <- ggplot(plot_data, aes(x = Population, y = Ratio, fill = Target_pop)) +
    ...
    labs(x = "Focal Population", y = "(tGLroh/tGLhet)/(rmGLroh/rmGLhet)", fill = "Reference Population") +
