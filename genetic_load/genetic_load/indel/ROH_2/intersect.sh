...
for i in $sBed/FJ*.HET.bed $sBed/PT*.HET.bed; do
	...
	for j in $sBed/MHKqns*.HET.bed $sBed/SZ*.HET.bed $sBed/JX*.HET.bed; do
		...
		bedtools intersect -a allpops.refMHK.SV.polarized.out.filtered.bed -b $ID1.$ID2.HET.bed > allpops.refMHK.SV.filtered.$ID1.$ID2.HET.bed
	done
done
...
for i in $sBed/FJ*.ROH.bed $sBed/PT*.ROH.bed; do
        ...
        for j in $sBed/MHKqns*.ROH.bed $sBed/SZ*.ROH.bed $sBed/JX*.ROH.bed; do
                ...
                bedtools intersect -a allpops.refMHK.SV.polarized.out.filtered.bed -b $ID1.$ID2.ROH.bed > allpops.refMHK.SV.filtered.$ID1.$ID2.ROH.bed
        done
done
