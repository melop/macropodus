#------------------------------------------------------------------
# 设置工作目录
setwd("/data2/projects/zwang/m.hk/Snpeff/all_MHK_MOP/rho_V2.0/filtered_SOL_3.5/Individual/indel/GL_VS_DP")
...
# 读取表格数据
table1 <- read.table("genetic_load_of_individual.allpops.refMHK.txt", header = T)
...
p <- ggplot(all_individuals_data, aes(x = DP, y = genetic_load)) +
  ...
  annotate("text", x = mean(range(all_individuals_data$DP)), y = max(all_individuals_data$genetic_load) * 0.8, label = "p=0.2428", size = 8) +
  scale_color_manual(values = color_mapping)


print(p)
# 保存为 PDF
ggsave("Linear_regression_analysis_between_geneticload_DP.group_color.pdf", plot = p, width = 12, height = 8)

#-------------------------------------------------------------------------------------混合线型模型。summary(model),将t value转换为斜率p value, p = 2*(1-pnorm(abs(t)))。0.2428
